...
namespace PS_Portal_Api.Controllers
{
    ...
    public class ComplaintControllerV1 : ControllerBase
    {
        ...
        public async Task<ResponseModelClass> GetSingleComplainData([FromBody] ComplaintViewRequest request)
        {
            ...
            return await _dataClass.GetSingleComplainData(request);
        }

        [HttpGet]
        [Route("GetAllContacts")]
        public async Task<ResponseModelClass> GetAllContacts()
        {

            return await ( _dataClass.GetAllContactsData());
        }

        [HttpPost]
        [Route("GetSingleContactsData")]
        public ResponseModelClass GetSingleContactsData(int request)
        {

            return _dataClass.GetContactsData(request);
        }

        [HttpPost]
        [Route("RegisterContacts")]
        public async Task<ResponseModelClass> RegisterContacts(ContactsUploadRequest request)
        {

            return await _dataClass.RegisterContact(request);
        }
    }
}
